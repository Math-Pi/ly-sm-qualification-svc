package com.ly.cloud.common.exception;
...
import com.ly.zhxg.utils.NHStringUtils.TranslateType;
...
public class NHExpHandleUtils {
	 ...
	 * @param e
	 */
	public static void throwesException(Exception e) {
		...
		if(t instanceof SQLException){
			...
			if(i==1){
				...
				List<Map<String,Object>> list= utils.mapper.selectListBySql(sql);
				String fields="";
				for(Map<String,Object> map:list){
					String columnName=(String)map.get("COLUMN_NAME");
					String fieldName=NHStringUtils.translateString(columnName, TranslateType.upper_to_hump);
					fields+=(","+fieldName);
				}
				throw new CloudException(fields.substring(1));
			}else if(i==12899){
				...
				throw new CloudException("dataToLongError["+fieldName+","+realLength+","+maxLength+"]",e);
				...
				String fieldName=NHStringUtils.translateString(columnName, TranslateType.upper_to_hump);
				throw new CloudException("columnNotNull["+fieldName+"]",e);
			}else if(i==2292){ //违反了约束条件，查询到了已使用的子记录
				throw new CloudException("businessLogicError[违反了约束条件，查询到了已使用的子记录！]",e);
			}else if(i==2291){//违反约束条件，未找到父项关键字
				throw new CloudException("businessLogicError[违反约束条件，未找到父项关键字！]",e);
			}else{
				throw new CloudException("businessLogicError[数据库异常，请联系管理员！]",e);
			}
		}else{//其他不知道是什么错误的异常，也直接抛出
			throw new CloudException(t.getMessage(),e);
		}
	}
